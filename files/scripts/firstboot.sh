...
if [[ -d "/boot/broadcom" ]]; then
	if [[ "$DISTRO" == "Devuan" ]]; then echo -e " \033[1mRunning fsck on boot partition\033[0m ..."; fi
...
if [[ "$ARCH" == "arm" ]]; then
	...
	if [[ `findmnt -v -n -o FSTYPE / | grep -w "ext4"` ]]; then ROOTFSTYPE="rootfstype=ext4"; fi
	if [[ `findmnt -v -n -o FSTYPE / | grep -w "btrfs"` ]]; then ROOTFSTYPE="rootfstype=btrfs rootflags=subvol=@"; fi
	if [[ `findmnt -v -n -o FSTYPE / | grep -w "xfs"` ]]; then ROOTFSTYPE="rootfstype=xfs"; fi
